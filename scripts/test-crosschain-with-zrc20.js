...
async function testCrosschainWithZRC20() {
  console.log('🔍 Testing cross-chain transfer with ZRC20 tokens...\n');
  ...
  const domainName = "testwithzrc20" + Date.now();
  ...
  try {
    ...
    console.log('\n📝 Step 2: Checking ZRC20 token balance on ZetaChain...');
    
    // Check ZRC20 balance on ZetaChain
    ...
    const zrc20Balance = await zrc20Contract.balanceOf(deployer.address);
    const symbol = await zrc20Contract.symbol();
    
    console.log(`ZRC20 Balance: ${ethers.formatEther(zrc20Balance)} ${symbol}`);
    
    if (zrc20Balance > 0) {
      console.log('✅ ZRC20 tokens available - cross-chain transfer should work');
    } else {
      console.log('❌ No ZRC20 tokens - cross-chain transfer may fail');
    }
    
    console.log('\n📝 Step 3: Attempting cross-chain transfer to Ethereum Sepolia...');
    ...
    console.log('\n📝 Step 4: Waiting for cross-chain processing (30 seconds)...');
    await new Promise(resolve => setTimeout(resolve, 30000)); // Wait 30 seconds
    
    console.log('\n📝 Step 5: Checking domain status after transfer...');
    ...
    if (arbIsAvailable && !ethIsAvailable) {
      ...
      console.log('✅ ZRC20 tokens enabled cross-chain processing');
      ...
      console.log('❌ ZRC20 tokens may not be sufficient or Gateway issue');
    } else {
