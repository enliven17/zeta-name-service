...
async function testSimpleCrossChain() {
  console.log('🌉 Testing Simplified Universal App Cross-Chain Transfer...\n');
  ...
  // Simplified Universal App contract addresses
  const arbSimpleAddress = "0xE0AA86abfb00A6E33EA4d94C00eA3B8E06C8477f";
  const ethSimpleAddress = "0x23BA3d4cFca85a3ff26C6FA32a2fac8B79f74432";
  ...
  try {
    ...
    const simpleAddress = currentChainId === 421614 ? arbSimpleAddress : ethSimpleAddress;
    const targetChainId = currentChainId === 421614 ? 11155111 : 421614;
    const targetSimpleAddress = currentChainId === 421614 ? ethSimpleAddress : arbSimpleAddress;
    
    console.log(`🔧 Using Simplified Universal App: ${simpleAddress}`);
    console.log(`🎯 Target Chain: ${targetChainId}`);
    console.log(`🎯 Target Universal App: ${targetSimpleAddress}`);

    const simpleContract = new ethers.Contract(simpleAddress, ABI, deployer);

    // Step 1: Register a test domain
    console.log('\n📝 Step 1: Registering test domain...');
    const domainName = "simplecrosschain";
    const isAvailable = await simpleContract.isAvailable(domainName);
    ...
    if (isAvailable) {
      ...
      const registerTx = await simpleContract.register(domainName, true, { // true = omnichain
    ...
    const domainInfo = await simpleContract.getDomainInfo(domainName);
    ...
    try {
      const crossChainTx = await simpleContract.crossChainTransfer(
        domainName,
        deployer.address,
        targetChainId,
        {
      ...
      const domainInfoAfter = await simpleContract.getDomainInfo(domainName);
      ...
      const targetContract = new ethers.Contract(targetSimpleAddress, ABI, targetProvider);
    ...
    console.log('====================================');
    console.log('✅ Simplified Universal App deployed on both chains');
